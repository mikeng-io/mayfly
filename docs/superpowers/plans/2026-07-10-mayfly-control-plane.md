...
- [ ] Add to stack: `NodejsFunction` + **Function URL** (auth NONE — HMAC is the auth). **Accepted v1 tradeoff, not best practice** — the hardened end state is CloudFront + WAF → Function URL via OAC; see `docs/adr/0002-webhook-ingress.md`. Env, grant SQS send + SSM read.
